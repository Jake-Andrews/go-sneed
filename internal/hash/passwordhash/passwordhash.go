...
func (h *PasswordHash) GenerateFromPassword(password string) (encodedHash string, err error) {
	...
	hash := argon2.IDKey([]byte(password), salt, h.iterations, h.memory, h.threads, h.keyLen)
	...
	encodedHash = fmt.Sprintf("$argon2id$v=%d$m=%d,t=%d,p=%d$%s$%s", argon2.Version, h.memory, h.iterations, h.threads, b64Salt, b64Hash)
